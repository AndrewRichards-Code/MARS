...
namespace mars
{
	//Multiplies a Vec2 input by a Mat2 transform.
	inline Vec2 Mat2Vec2Multiply(const Vec2& input, const Mat2& transform)
	{
		float x = input.x;
		float y = input.y;
	...
	{
		return Mat2Vec2Multiply(input, transform);
	}
	//Operator Overload * , using the Mat2Vec2Multi().
	//Multiplies a Vec2 input by a Mat2 transform.
	...
	//Multiplies a Vec3 input by a Mat3 transform.
	inline Vec3 Mat3Vec3Multiply(const Vec3& input, const Mat3& transform)
	{
		float x = input.x;
		float y = input.y;
		float z = input.z;
	...
	{
		return Mat3Vec3Multiply(input, transform);
	}
	//Operator Overload * , using the Mat3Vec3Multi().
	//Multiplies a Vec3 input by a Mat3 transform.
	...
	inline Vec4 Mat4Vec4Multiply(const Vec4& input, const Mat4& transform)
	{
		float x = input.x;
		float y = input.y;
		float z = input.z;
	...
	//Multiplies a Vec4 input by a Mat4 transform.
	inline Vec4 operator* (const Vec4& input, const Mat4& transform)
	{
		return Mat4Vec4Multiply(input, transform);
	}
	//Operator Overload * , using the Mat4Vec4Multi().
	//Multiplies a Vec4 input by a Mat4 transform.
